...
The Holographic Circlette: Part~II\\
Composites, Decays, and the Zero-Sum Identity
}}}\end{center}
...
\textbf{\boldmath{%
We extend the circlette lattice model---in which 45 Standard Model fermions
correspond to valid codewords of an $[8,4]$ binary error-correcting code---to
...
\begin{itemize}[nosep]
  \item[\textbf{R1.}] $\chi = W$ \quad (chirality self-consistency).
  \item[\textbf{R2.}] If $LQ = 0$ (lepton), then $C_0 = C_1 = 0$ \quad (leptons are colourless).
  \item[\textbf{R3a.}] If $LQ = 1$ (quark), then $C_0 C_1 \neq 00$ \quad (quarks must carry colour).
  \item[\textbf{R3b.}] Equivalently: a colourless quark is forbidden.
...
The companion papers established the static encoding and its dynamical consequences: gauge bosons as ring operators, mass hierarchy from lattice criticality, gravity from Fisher information geometry, and the CKM matrix as Hamming distance on the codeword space.  This paper extends the framework to \emph{composite} particles and shows that the code's arithmetic yields an exact identity encoding all Standard Model conservation laws.
...
The composite \cw{00100100} has $LQ = 1$ (quark) but $C_0 C_1 = 00$ (colourless).  This violates constraint R3b---it is a colourless quark.  The composite is not a valid codeword.
...
Computational enumeration over all chirality and colour-permutation combinations confirms that \emph{every} colour-neutral three-quark composite violates exactly one constraint (R3b), producing a colourless quark.  All composites sit at Hamming distance~1 from the corresponding lepton:
...
A baryon is a \textbf{single-error state} of the code.  It violates one parity check and sits one bit-flip from the valid codeword space.  The error-correction dynamics of the lattice perpetually attempt to restore it to the nearest valid codeword.  The required correction is a flip of the $LQ$ bridge bit---precisely the weak CNOT gate.
...
The correct formulation connects directly to the Fisher Information Metric introduced in Part~III of the companion paper.  The local correction load deforms the \emph{probability distribution} of syndrome updates at each lattice site.  If $p_\theta(s)$ is the probability of observing syndrome $s$ at a site parametrised by local conditions $\theta^\mu$ (mass density, momentum flow, stress), then the Fisher Information Matrix
\begin{equation}
...
The programme continues: systematic verification of the zero-sum across all Standard Model vertices, non-perturbative QCD calculation of the gluon dressing factor for the up-quark structure factor, experimental tests of the entanglement monogamy bound, derivation of molecular geometry from ring-tiling constraints, and the full derivation of Einstein's equations from the Fisher information metric of the lattice's syndrome statistics.

% ============================================================
\section*{Acknowledgements}
% ============================================================

The author thanks Claude (Anthropic) for extensive assistance with analysis,
computational verification, and document preparation.  The circlette
encoding, ring topology, parity-check constraints, and physical
interpretations are the author's own.  All errors remain the author's
responsibility.
