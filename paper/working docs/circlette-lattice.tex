...
% Simplified schematic of the 4.8.8 tiling
\def\d{2.414} % 1 + sqrt(2)
\def\s{0.9}   % octagon visual size

% Draw plaquettes (squares) first
\foreach \i in {0,1} {
  \foreach \j in {0,1} {
    \pgfmathsetmacro{\px}{\i*\d + \d/2}
    \pgfmathsetmacro{\py}{\j*\d + \d/2}
    \fill[green!20, draw=green!50!black, line width=0.5pt] 
      (\px-0.3, \py-0.3) rectangle (\px+0.3, \py+0.3);
  }
}
...
\foreach \i in {0,1,2} {
  \foreach \j in {0,1,2} {
    \pgfmathsetmacro{\cx}{\i*\d}
    \pgfmathsetmacro{\cy}{\j*\d}
    % Right link
    \ifnum\i<2
      \pgfmathsetmacro{\nx}{(\i+1)*\d}
      \draw[blue!60!black, line width=1.5pt] (\cx+\s, \cy) -- (\nx-\s, \cy);
      \fill[blue!60!black] ({(\cx+\nx)/2}, \cy) circle (2pt);
    \fi
    % Up link
    \ifnum\j<2
      \pgfmathsetmacro{\ny}{(\j+1)*\d}
      \draw[blue!60!black, line width=1.5pt] (\cx, \cy+\s) -- (\cx, \ny-\s);
      \fill[blue!60!black] (\cx, {(\cy+\ny)/2}) circle (2pt);
    \fi
  }
}
...
\foreach \i in {0,1,2} {
  \foreach \j in {0,1,2} {
    ...
    \pgfmathsetmacro{\isexcited}{(\i==1 && \j==1) ? 1 : 0}
    \ifnum\isexcited=1
      \draw[fill=red!30, draw=red!60!black, line width=1pt] 
        (\cx,\cy) ++(22.5:\s) \foreach \a in {67.5,112.5,...,337.5} { -- ++(\a:\s*0.765) } -- cycle;
    \else
      \draw[fill=orange!15, draw=gray, line width=1pt] 
        (\cx,\cy) ++(22.5:\s) \foreach \a in {67.5,112.5,...,337.5} { -- ++(\a:\s*0.765) } -- cycle;
    \fi
  }
}
...
% Labels
\node[font=\small\bfseries] at (0, -0.0) {vac};
\node[font=\small\bfseries, text=red!60!black] at (\d, \d) {$\psi$};
\node[font=\small\bfseries] at (2*\d, 0) {vac};

% Legend
\node[anchor=west, font=\footnotesize] at (5.5, 4.5) {$\bigcirc$ Circlette (8-bit register)};
\node[anchor=west, font=\footnotesize, blue!60!black] at (5.5, 3.8) {$\bullet\!\!-\!\!\bullet$ Link (comm.\ channel, $\UU(1)$ phase)};
\node[anchor=west, font=\footnotesize, green!50!black] at (5.5, 3.1) {$\square$ Plaquette (Wilson loop)};
...
\begin{equation}
  ...
  + \underbrace{4t_2 \sin k_x \sin k_y \cdot \alpha_1\alpha_2}_{\text{NNN (diagonal)}}.
\label{eq:H488}
\end{equation}
...
In the theory of discrete-time quantum walks (DTQWs)~\citep{Venegas2012,DAriano2014}, the coin operator - the internal-state update applied at each tick before the conditional shift - is the sole generator of the effective mass in the continuum limit. In the \circlette\ framework, the coin operator is the \textbf{CNOT gate}:
\begin{equation}
...
\item A discrete binary state space does not support the topological winding required by the theorem.
\end{enumerate}
