...
def build_entries() -> list[Entry]:
    ...
    return [
        ...
        Entry(
            ...
            prediction="r approximately 2e-9; no observable primordial B-modes",
            current_status=(
            ...
            canon_hook="papers/boundary_printing/boundary_printing.tex; python_code/correlated_nulls_ledger.py",
        ),
        Entry(
...
def build_markdown(entries: list[Entry]) -> str:
    ...
    return "\n".join(
        [
            ...
            "the proton-primary `G/H0` route, an essentially zero primordial-tensor",
            "prediction `r ~ 2e-9`, and the global trans-`Lambda_QCD` Lorentz bridge.",
...
def main() -> None:
    ...
    assert any("r approximately 2e-9" in entry.prediction for entry in entries)
