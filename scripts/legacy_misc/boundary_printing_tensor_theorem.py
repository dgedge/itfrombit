...
NO transverse-traceless (spin-2) mode. The only tensor channel left is SECOND-order
scalar-induced gravitational waves, with r_SIGW ~ O(A_s) ~ 1e-9 -- ten orders below r_naive.
...
THE TWO BRANCHES, made mutually exclusive by the event unit:
  ...
  * Branch B (no squeezing; discrete printer, scalar shot noise): the one-bit event unit stands,
    H_* = 1.2e15, and tensors sit at the second-order floor r ~ 1e-9.
...
print("    Only 2nd-order scalar-induced GWs remain, at the floor:")
r_sigw = A_S                      # order-of-magnitude 2nd-order floor: P_t^(2) ~ A_s^2 => r ~ A_s
print(f"      r_SIGW ~ O(A_s) ~ {r_sigw:.1e}   (~{r_naive / r_sigw:.0e}x below r_naive; ~{R_BOUND / r_sigw:.0e}x below the bound)")
assert r_sigw < R_BOUND
...
print(f"      tensors at the 2nd-order floor r ~ {r_sigw:.0e}.")
...
[verdict] THE EXPOSURE IS CONDITIONALLY DEFUSED -- downgraded from a clean falsifier to one that
  ...
  * The printer's mechanism is SCALAR shot noise; by SVT decoupling it sources no linear spin-2
    mode, leaving only 2nd-order induced GWs at r ~ {r_sigw:.0e} -- a concrete prediction of
    essentially ZERO observable primordial tensors.
  * The one-bit event unit forces Branch B; Branch A would need ~{u_event_A / U_LN2:.0f} bits/cell, contradicting
  ...
  THE DECISIVE TEST: r. The framework predicts r ~ {r_sigw:.0e} (no observable B-modes). A detection
